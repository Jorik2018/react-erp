...
const initActivityProgress = (activity, activityProgress) => async (dispatch) => {
	...
	const foundIndex = activity.cards.findIndex(
		(card:{id:string}) => card.id === activityProgress.lastCardUnlockedId
	)
...
export const initUnlockCard = (activityId:string, id:string) => async (dispatch) => {
	dispatch(
		saveToCache(
	...
	unlockCard(activityId, id).then((res:{message:string}) => console.log(res.message))
}

export const initUnlockHint = (activityId: string, id: string) => async (dispatch) => {
	dispatch(
		saveToCache(
			CACHE_HINT_PROGRESS,
			{ [id]: { isUnlocked: true } },
	...
	unlockHint(activityId, id).then((res:{message:string}) => console.log(res.message))
}
...
export const setCurrentCardByIndex = (cardIndex:string) => ({
...
export const setLastCardUnlockedIndexById = (cardId:string) => ({

...
export const fetchTeacherData = (teacherId:string) => {
	const endpoint = `teachers/${teacherId}`
	return backend.get(endpoint)
}

export const fetchClassroom = (classroomId:string) => {
	const endpoint = `classrooms/${classroomId}`
	return backend.get(endpoint)
}

export const fetchSubmissionsAll = (classroomId:string) => {
 ...
 * }
 */
export const gradeSubmission = (activityId:string, data) => {
...
class TeacherService {
	async getSubmissions(classroomID:string) {
		const endpoint = `/teachers/${classroomID}/grade`
		return backend.get(endpoint)
	}

	async submitFeedbacks(classroomID:string, submissionID:string, feedbacks) {
		const endpoint = `/teachers/${classroomID}/grade`

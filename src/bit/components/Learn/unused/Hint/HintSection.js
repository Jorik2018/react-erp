...
class HintSection extends Component {
	...
	componentDidUpdate(prevProps) {
		if (this.props.cardID !== prevProps.cardID) {
			...
			getHintStatus(this.state.labID, this.state.cardID)
				.then(rawData => processHintStatus(rawData))
				.then(data => {
					this.setState({
						hints: data
					})
	...
	render() {
		...
		return <HintArea>{hints}</HintArea>
	}
}

...
export const CodeBlock = ({ language, value, style, expandBlocks }: CodeBlockProps) => {
	return (
		<SyntaxHighlighter
...
const MarkdownContent = ({ className, source, options = initialOptions }:
	{ className: string, source: string, options: Obj }) => {
	const { expandBlocks } = { ...initialOptions, ...options }

	const renderers = {
		code: (props: CodeBlockProps) => CodeBlock({ ...props, expandBlocks }),
		image: Image,
		blockquote: (props: Obj) => Blockquote({ ...props, expandBlocks }),
		table: Table,
		tableRow: TableRow,
		tableCell: TableCell
	}

...
export const Search = ({ onSelect = () => { }, show }:
    { onSelect: (item?: Oemsecret) => void, show: boolean }) => {
    ...
    const oemsecretRemap = (obj: {
        stock: {
            ...
            distributor: { distributor_name: string, distributor_logo: string },
            sku: string,
            manufacturer: string,
            part_number: string
        }[]
    }): Oemsecret[] => obj.stock.map((x) => ({
        link: x.buy_now_url,
        name: x.description,
        image: x.image_url,
    ...
    const onEnter = (term?: string) => {
        ...
        fetch('/api/oemsecret/' + (term || state.searchTerm))
            ...
            .then(data => {
                setState({
                    ...
                    result: {
                        ...
                        distributors: [...new Set(oemsecretRemap(data).map((item: Oemsecret) => {
                ...
                {state.result.oemsecret && !state.loading && (
                    <>
                        ...
                        <div className="js-filter distributor--result uk-list uk-list-striped uk-list-divider">
                            {
                                state.result.oemsecret.map((item: Oemsecret) => {
                                    // TODO: Should we enable ? Items without descr. or just a numeric descr.
                                    // const reg = /^\d+$/;
                                    // if (!item.description) return false;
                                    // if (reg.test(item.description)) return false;
                                    return (
                                        <ul key={createId()} className="distributor--item" data-distributor={item.supplier}>

...
import { ADMIN_LEVEL, ITEM_SAVED, NEED_TITLE, ADMIN_REQUIRED, CONFIRM_DELETE, ITEM_REMOVED } from '../constants/constants';
...
class Admin extends Component {
  ...
  save() {
    if (this.props.userData.info.level === ADMIN_LEVEL) {
      ...
      if (item && (item.title || this.state.type === 'users')) {
        ...
        if (!uploadFile) {
          this.props.firebase[method](path, item).then((snap) => {
            ...
            this.props.setNotification({ message: ITEM_SAVED, type: 'success' });
        ...
        this.props.setNotification({ message: NEED_TITLE, type: 'error' });
      }
    } else {
      this.props.setNotification({ message: ADMIN_REQUIRED, type: 'error' });
    }
  }
  ...
  delete() {
    this.setState({
      modalTitle: `${CONFIRM_DELETE} "${this.state.selectedItem.title}"?`
    }, () => {
  ...
  updateItem(value: string, prop: string, callback) {
    let newItem = null;
      ...
      if (this.state.selectedItem.file) {
        ...
        desertRef.delete().then(() => {
          this.props.firebase.remove(`${this.state.type}/${this.state.selectedId}`, () => {
            this.toggleButtons(true);
            this.cancel();
            this.props.setNotification({ message: ITEM_REMOVED, type: 'success' });
          });
        }).catch((error) => {
          this.props.setNotification({ message: error, type: 'error' });
        });

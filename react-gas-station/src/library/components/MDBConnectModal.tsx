...
export const ConnectModal = ({
  ...
  const { account, error, activateBrowserWallet, activateWalletConnect } =
    useEthers();
  ...
  return (
    <MDBModal show={connectModal} setShow={setConnectModal} tabIndex="-1">
      <MDBModalDialog centered>
        <MDBModalContent>
          <MDBModalBody>
            {account ? (
              <div className="text-center">
                Connected as: {shortenString(account)}
              </div>
            ) : (
              <div className="">
                {errorMessage ? (
                  <MDBAlert show className="w-100 text-center" color="danger">
                    {errorMessage}
                  </MDBAlert>
                ) : (
                  <></>
                ...
                <MDBBtn
                  color="link"
                  onClick={() =>
                    activateWalletConnect(undefined, true)
                      .then(() => setConnectModal(false))
                      .catch(setConnectError)
                  }
                  className="w-100"
